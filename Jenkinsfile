pipeline{
    agent any
       ...
       environment {
            DEPLOY = "${env.BRANCH_NAME == "python-monitor" || env.BRANCH_NAME == "master" ? "true" : "false"}"
            NAME = "${env.BRANCH_NAME == "python-monitor" ? "example" : "example-staging"}"
            //def mavenHome =  tool name: "maven:11.0.16", type: "maven"
            //def mavenCMD = "${mavenHome}/usr/share/maven"
            VERSION = "${env.BUILD_ID}"
            REGISTRY = 'eagunuworld/python-monitor'
          ...
          stage('Building Docker Images') {
            agent {
                ...
                steps {
                  ...
                  sh "docker build . -t ${REGISTRY}:${VERSION} -f build/Dockerfile"
                     }
                 }
